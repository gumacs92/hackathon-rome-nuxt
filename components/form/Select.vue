<template>
  ...
  >
    ...
    <label
      ...
      :class="[{
        'translate-y-3 text-base ': selectedItems.length === 0
      }, labelClass]"
      :for="id"
    >{{ placeholder }}</label>
    <!--
    ...
    <button
      id="dropzone"
      class="h-12 px-5 w-full text-sm outline-none focus:outline-none bg-glass-100  border rounded-xl
    ...
    <div
      ...
      class="z-30 absolute top-14 bg-black shadow-box rounded-lg p-9 w-full h-auto transition-height duration-500 overflow-y-auto"
    >
      <OverlayLoader
        :loading="loading"
        ...
        <div v-else>
          <div v-if="showSelected">
            ...
            <div
              v-for="(item, index) in selectedItems"
              :key="'selected_'+ index + (item.key ? item.key : item.value)"
              class="text-primary pointer-events-auto cursor-pointer"
